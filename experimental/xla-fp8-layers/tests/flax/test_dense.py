...
class DenseTest(jtu.JaxTestCase):
  ...
  def testDenseFwd(self):
    ...
    infer_fn_ref = jit(partial(_infer, dense_ref))
    y_ref = infer_fn_ref(variables_ref, x)

    infer_fn = jit(partial(_infer, dense))
  ...
  def testDenseBwd(self) :
    ...
    train_fn_ref = jit(
        value_and_grad(partial(_train_loss, dense_ref), argnums=[0, 1]))
    loss_val_ref, grads_ref = train_fn_ref(variables_ref, x)

    train_fn = jit(
        value_and_grad(partial(_train_loss, dense), argnums=[0, 1]))
  ...
  def testDenseBwdHlo(self):
    ...
    def _train_step(params, x_data):
      ...
      loss_grad_fn = value_and_grad(loss_fn, argnums=[0, 1])
  ...
  def testDenseDenseFwdHlo(self):
    for use_fp32 in [False, True]:
      ...
      hlo = jit(predict).lower(variables, x).compile()
      self.assertRegex(
          hlo.as_text(), re.compile('.*'.join([re.escape(x) for x in (
              'cublas',
              'f8e4m3fn[16,32]{1,0}',
  ...
  def testDenseDenseBwdHlo(self):
    for use_fp32 in [True, False]:
      ...
      def _train_step(params, x_data):
        ...
        loss_grad_fn = jit(value_and_grad(loss_fn, argnums=[0, 1]))
        loss, gradients = loss_grad_fn(params, x_data)
        return loss, gradients
      ...
      hlo = jit(_train_step).lower(variables, x).compile()
